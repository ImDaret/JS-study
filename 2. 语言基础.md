...
1.  `~`: 按位非，1 变 0，0 变 1。
2.  `｜`: 按位或，有 1 变 1，没 1 变 0。
3.  `&`: 按位与，两个 1 为 1，其余 0。
4.  `^`: 按位异或，两位只有一个 1 为 1，其余为 0。
5.  `<<`: 按位左移，用 0 填充，例如`2<<5 === 64`,符号位不变
6.  `>>`: 有符号右移，用 0 填充，例如`64>>5 === 2`,符号位不变
7.  `>>>`: 无符号右移，用 0 填充，符号位会变，因为用 0 填充，所以对正数来说是一样的，对于负数来说差别可能很大。
...
如上所示，labelLan 是一个标签，可以搭配`break`,`continue`语句控制父子循环。
